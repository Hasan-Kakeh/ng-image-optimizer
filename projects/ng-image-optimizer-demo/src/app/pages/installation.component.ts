...
@Component({
  ...
  template: `
    ...
    <h2>Manual Setup</h2>
    <p>If you prefer to configure things manually, first install the library and its peer dependencies:</p>
    <app-code-block language="bash" code="npm install ng-image-optimizer sharp"></app-code-block>
    
    <h2>2. Verify Configuration</h2>
    <p>The schematic automatically configures the Angular Application and your Express server. However, you can verify it manually:</p>

    <h3>Application Configuration (<code>app.config.ts</code>)</h3>
    <p>Ensure the provider function is added to your app config to register the custom optimized image loader:</p>
    <app-code-block language="typescript" [code]="appConfigCode"></app-code-block>

    <h3>Server Middleware (<code>server.ts</code>)</h3>
    <p>The optimizer works as an Express middleware intercepting paths (defaulting to <code>/_ng/image</code>).</p>
...
// Set up the optimizer middleware BEFORE Angular Universal handlers
